            ...
            {items.filter(i=>i.category===cat).map((item,idx,arr)=>{
              ...
              return (
                <div key={item.id} style={{padding:'0.7rem 1rem',borderBottom:idx<arr.length-1?'1.5px solid var(--outside-cream)':'none'}}>
                  ...
                  {item.name==='Café' && <div style={{fontSize:'0.6rem',color:'red'}}>inv:{item.stockDebug_debut} rec:{item.stockDebug_recu} conso:{item.stockDebug_conso} pertes:{item.stockDebug_perdus}</div>}
                    </div>
                  </div>
