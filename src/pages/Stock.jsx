...
function TabMouvements({ isManager, profile }) {
  ...
  async function loadData() {
    ...
    const [{ data: si }, { data: mvt }, { data: pertes }] = await Promise.all([
      supabase.from('stock_items').select('*').eq('active',true).order('category').order('name'),
      supabase.from('stock_movements').select('*,stock_items(name,unit)').eq('type','reception').order('created_at',{ascending:false}).limit(30),
      supabase.from('stock_pertes').select('*').order('date_perte',{ascending:false}).order('created_at',{ascending:false}).limit(30),
    ])
  ...
  return (
    <>
      ...
      {loading ? <div style={{display:'flex',justifyContent:'center',padding:'2rem'}}><Spinner size={24}/></div> : (
        <div className="card">
          {mouvements.length===0 ? (
            <div style={{padding:'2rem',textAlign:'center',color:'var(--muted)'}}>Aucun mouvement enregistré</div>
          ) : mouvements.map((m,idx)=>{
            const isReception = m._type==='reception'
            const motif = !isReception ? MOTIFS.find(x=>x.value===m.motif) : null
            return (
